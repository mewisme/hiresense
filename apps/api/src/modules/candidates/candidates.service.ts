...
export type CandidateProfile = NonNullable<
  Awaited<ReturnType<CandidateProfilesRepository['findByUserId']>>
>;

@Injectable()
export class CandidatesService {
  constructor(private readonly candidateProfilesRepository: CandidateProfilesRepository) { }
  ...
  async findByUserId(userId: string): Promise<CandidateProfile | null> {
    return this.candidateProfilesRepository.findByUserId(userId);
  }
  ...
  async createMyProfile(
    ...
    try {
      return await this.candidateProfilesRepository.create({
        userId,
        ...
        experienceMonthsDeclared: dto.experienceMonthsDeclared ?? undefined,
  ...
  async updateMyProfile(
    ...
    try {
      return await this.candidateProfilesRepository.updateByUserId(userId, dto);
    } catch (error) {
      if (
        error instanceof PrismaNamespace.PrismaClientKnownRequestError &&
        error.code === 'P2025'
